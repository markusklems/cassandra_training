package de.cassandratraining.netflix;

// Copied from https://raw.github.com/Netflix/astyanax/master/astyanax-examples/src/main/java/com/netflix/astyanax/examples/AstCQLClient.javaF
...
public class AstCQLClient {
	...
	private static final String INSERT_STATEMENT = String.format(
			...
			EMP_CF_NAME, ModelConstants.COL_NAME_EMPID, ModelConstants.COL_NAME_DEPTID, ModelConstants.COL_NAME_FIRST_NAME,
			ModelConstants.COL_NAME_LAST_NAME);
	private static final String CREATE_STATEMENT = String
			.format("CREATE TABLE %s (%s int, %s int, %s varchar, %s varchar, PRIMARY KEY (%s, %s))",
					EMP_CF_NAME, ModelConstants.COL_NAME_EMPID, ModelConstants.COL_NAME_DEPTID,
					ModelConstants.COL_NAME_FIRST_NAME, ModelConstants.COL_NAME_LAST_NAME, ModelConstants.COL_NAME_EMPID,
					ModelConstants.COL_NAME_DEPTID);

	public void init() {
		logger.debug("init()");
		
		String seeds = System.getenv("CASSANDRA_HOST")+":9160";
	...
	public void read(int empId) {
		...
		try {
			OperationResult<CqlResult<Integer, String>> result = keyspace
					...
					.withCql(
							String.format("SELECT * FROM %s WHERE %s=%d;",
									EMP_CF_NAME, ModelConstants.COL_NAME_EMPID, empId))
					.execute();
			for (Row<Integer, String> row : result.getResult().getRows()) {
				...
				logger.debug("emp");
				logger.debug("- emp id: "
						+ cols.getIntegerValue(ModelConstants.COL_NAME_EMPID, null));
				logger.debug("- dept: "
						+ cols.getIntegerValue(ModelConstants.COL_NAME_DEPTID, null));
				logger.debug("- firstName: "
						+ cols.getStringValue(ModelConstants.COL_NAME_FIRST_NAME, null));
				logger.debug("- lastName: "
						+ cols.getStringValue(ModelConstants.COL_NAME_LAST_NAME, null));
			}
		} catch (ConnectionException e) {
